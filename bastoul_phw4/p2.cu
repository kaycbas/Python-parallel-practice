...
__global__ void shared_matrix_mult(Matrix a, Matrix b, Matrix c) {
	...
	for (int i=0; i<n/BLOCK_SIZE; i++) {
		a_s[row][col] = a.elements[id_x*n + (i*blockDim.y + col)];
		b_s[row][col] = b.elements[(i*blockDim.x+row)*n + id_y];
		__syncthreads();
...
int main () {
	...
	shared_matrix_mult<<<dimGrid, dimBlock>>>(gpu_a, gpu_b, gpu_c);
